 ...
 **********************/
#if DM_CUSTOM == 0
//static DM_MEM_ATTR uint8_t work_mem[DM_MEM_SIZE] __attribute__ ((at(SDRAM_BASE_ADDR + DYNMEM_ADDR)));    //Work memory for allocation
static DM_MEM_ATTR uint8_t work_mem[DM_MEM_SIZE] = {0};   //Work memory for allocation
#endif

static uint32_t zero_mem;       /*Give the address of this variable if 0 byte should be allocated*/ 

#ifndef SIMPRO
osMutexDef (DYNMemMutex); 
...
void dm_init(void)
{
	...
	#ifndef SIMPRO
	DYNMemMutex_id = osMutexCreate  (osMutex (DYNMemMutex));
	#endif
#endif
}
...
void * dm_alloc(uint32_t size)
{
	...
	#ifndef SIMPRO
    osMutexWait(DYNMemMutex_id, osWaitForever);
	#endif
...
{    
    ...
    e = (dm_ent_t *)((uint8_t *) data - sizeof(dm_header_t));
    e->header.used = 0;
