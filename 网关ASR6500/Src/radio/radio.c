...
/*!
 * \brief Generates a 32 bits random value based on the RSSI readings  ??RSSI??????32?????
 *
...
#ifdef _COSMIC_
@near const RadioLoRaBandwidths_t Bandwidths[] = 
{ LORA_BW_007, LORA_BW_010, LORA_BW_015, LORA_BW_020, LORA_BW_031, LORA_BW_041, LORA_BW_062, LORA_BW_125, LORA_BW_250, LORA_BW_500 };
...
{ LORA_BW_007, LORA_BW_010, LORA_BW_015, LORA_BW_020, 
 ...
 * Tx and Rx timers
 */
TimerEvent_t TxTimeoutTimer = {0};
TimerEvent_t RxTimeoutTimer = {0};

RadioStatistics_t gRadioStatistics;
...
//            ( ( bandwidth == 1 ) && ( datarate == 12 ) ) )
            if( ( ( bandwidth == 7 ) && ( ( datarate == 10 ) || ( datarate == 11 ) || ( datarate == 12 ) ) ) ||
            ( ( bandwidth == 8 ) && ( datarate == 12 ) )  ||
			( bandwidth <= 6)	)
            {
                SX126x.ModulationParams.Params.LoRa.LowDatarateOptimize = 0x01;
...
const double RadioLoRaBandwidthsReal[] = {
7.81, 15.63, 31.25, 62.5, 125, 250, 500, 0, 10.42, 20.83, 41.67
};
    ...
    case MODEM_LORA:
        {
			...
			double ts = pow(2, SX126x.ModulationParams.Params.LoRa.SpreadingFactor) / RadioLoRaBandwidthsReal[SX126x.ModulationParams.Params.LoRa.Bandwidth];
...
void RadioSend( uint8_t *buffer, uint8_t size )
{
	...
	if( SX126xGetPacketType( ) == PACKET_TYPE_LORA )
		tmpu32 = RadioTimeOnAir(MODEM_LORA, size)+ TxTimeout;
	else
		tmpu32 = RadioTimeOnAir(MODEM_FSK, size)+ TxTimeout;
    TimerSetValue( &TxTimeoutTimer, tmpu32 );
...
void RadioRx( uint32_t timeout )
{
//    SX126xSetDioIrqParams( IRQ_RX_DONE | IRQ_RX_TX_TIMEOUT | IRQ_HEADER_ERROR | IRQ_CRC_ERROR,
...
void RadioIrqProcess( void )
{
    ...
    if( IrqFired == true )
    {
        ...
        {
			if (TxTimeoutTimer.osTimerID != NULL)
		...
		else if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )
        {
            gRadioStatistics.RevCount++;
			//if (RxTimeoutTimer.osTimerID != NULL)
